...
public class Helpers {
    ...
    public static void reportRHSettings() {
        String msg = MessageFormat.format("\n# Settings:\n" +
                        "# Ins Length = {0} | CAR Length = {1}\n" +
                        "# Support = {2} | Confidence = {3}\n" +
                        "# Head Coverage = {4} | Learn Groundings = {5}\n" +
                        ...
                        "# Ins Rule Cap = {16} | Gen Time = {17}\n" +
                        "# Quality Measure = {18} | Overfitting Factor = {19}\n" +
                        "# Prior Prune = {20} | Prior Prune Threshold = {21}\n" +
                        "# Post Prune = {22} | Filter Unsolvable = {23}"
                , Settings.INS_DEPTH
                , Settings.CAR_DEPTH
                , Settings.SUPPORT
                , String.valueOf(Settings.CONF)
                ...
                , Settings.QUALITY_MEASURE
                , Settings.OVERFITTING_FACTOR
                , RHSettings.USE_PRIOR_PRUNE
                , RHSettings.PRIOR_PRUNE_TH
                , RHSettings.USE_POST_PRUNE
                , RHSettings.FILTER_UNSOLVABLE
        );
        Logger.println(msg, 1);
    }

    public static void reportLocalRuleAnalysis() {
        String msg = MessageFormat.format("# Abstract Rules: ARs = {0} | SARs = {1} | QARs = {2} | UARs = {3} | PARs = {4}\n" +
                "# Instantiated Rules: IRs = {5} | QIRs = {6} | UIRs = {7} | PIRs = {8}\n" +
                "# Time/Space Constraints Triggered: {9}"
                , RuleLogger.getARs()
                , RuleLogger.getSpecializedARs()
                , RuleLogger.getQualifiedARs()
                , RuleLogger.getUnqualifiedARs()
                , RuleLogger.getPriorPrunedARs()
                , RuleLogger.getIRs()
                , RuleLogger.getQualifiedIRs()
                , RuleLogger.getUnqualifiedIRs()
                , RuleLogger.getPostPrunedIRs()
                , RuleLogger.isConstraintsTriggered());
        Logger.println(msg);
    }

    public static void reportGlobalRuleAnalysis() {
        String msg = MessageFormat.format("# Abstract Rules: ARs = {0} | SARs = {1} | QARs = {2} | UARs = {3} | PARs = {4}\n" +
                        "# Instantiated Rules: IRs = {5} | QIRs = {6} | UIRs = {7} | PIRs = {8}\n" +
                        "# Time/Space Constraints Triggered: {9}"
                , RuleLogger.reportAvg(RuleLogger.ARs)
                , RuleLogger.reportAvg(RuleLogger.specializedARs)
                , RuleLogger.reportAvg(RuleLogger.qualifiedARs)
                , RuleLogger.reportAvg(RuleLogger.unqualifiedARs)
                , RuleLogger.reportAvg(RuleLogger.priorPrunedARs)
                , RuleLogger.reportAvg(RuleLogger.IRs)
                , RuleLogger.reportAvg(RuleLogger.qualifiedIRs)
                , RuleLogger.reportAvg(RuleLogger.unqualifiedIRs)
                , RuleLogger.reportAvg(RuleLogger.postPrunedIRs)
                , RuleLogger.constraintsTriggeredCount());
        Logger.println(msg);
    }
}
